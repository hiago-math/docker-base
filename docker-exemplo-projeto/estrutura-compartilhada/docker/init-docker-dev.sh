...
docker cp $PWD/docker/build/nginx/api-exemplo.conf api-exemplo-container:/etc/nginx/conf.d/

echo 'Copiando configuração do ngnix para o nginx base...'
docker cp $PWD/docker/build/nginx/api-exemplo.conf nginx:/etc/nginx/conf.d/
